...
#----------------------------PAIO-------------------------------
    ...
    def fit_sample(self, X, y):
        ...
        for i, index_i in enumerate(self.TS):
            ...
            CTh=[]
            for j, dist_j in enumerate(dist[self.min_index[0].index(index_i)][1:self.k1+1]):
                if dist_j<= dTh:
            ...
            for j, index_j in enumerate(CAR_i):
                dist_i_j=np.linalg.norm(X_removed[index_i]-X_removed[index_j])
    ...
    def overlap_ratio(self, X_re, y_re):
        min_i = np.where(y_re == 0)
        min_s = X_re[min_i]
        N = len(min_s)
        neigh = NearestNeighbors(n_neighbors=N).fit(min_s)
        dist, indices = neigh.kneighbors(min_s)
        K = int(np.ceil(self.w * N))
        dc = np.average(dist[:, K])
        p = np.zeros(N)
        for i in range(N):
            for j in range(1, N):
                p[i] += np.exp(-(dist[i, j] ** 2 / dc ** 2)) 
        density = p
        neigh = NearestNeighbors(n_neighbors=self.k1 + 1).fit(X_re)
        dist, indices = neigh.kneighbors(X_re)

        q = {}
        p = np.zeros(N)

        del_index_list = []
        for i, index in enumerate(min_i[0]):

            neigh_label = y_re[indices[index, 1:self.k1 + 1]]
        ...
        X_y = add_label(X_re, y_re)
        X_y_removed = np.delete(X_y, del_index_list, axis=0)
        X_removed= X_y_removed[:, :-1]


        overlap_num=len(X_re)-len(X_removed)
        return overlap_num/len(X_re)
